...
void pega_coloca_dados (char *entrada);



void main (int agrc, char *argv[]){
	pega_coloca_dados(argv[1]);
}
...
void imprimir (struct No *raiz, struct Lista **fila){
	FILE *doc = NULL;
	doc = fopen ("resposta.txt", "a");
	if (raiz == NULL){
		fprintf(doc, "X ");
		fclose(doc);
		return;
	}
	fprintf (doc, "%d ", raiz->numero);
...
void pega_coloca_dados (char *entrada){
	...
	if (arq == NULL){
		...
		fprintf (res, "Erro na leitura do arquivo ou o arquivo não existe");
		fclose(res);
		fclose(arq);
		return;
	}
