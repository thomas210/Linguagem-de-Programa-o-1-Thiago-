...
void executar (const char *reservas, const char *clientes, const char *frequencia);
...
void executar (const char *reservas, const char *clientes, const char *frequencia){
	...
	fclose(arq);
	arq = fopen(frequencia,"r");
	for (i = 0; i < 24; i++){
		fscanf(arq, "%lf ", &freq[i]);
	}
	fscanf(arq, "\n");
	fscanf(arq, "%d ", &total);
	fclose(arq);
	...
	while (!feof(arq)){
		...
		if (escolha == 'm'){
			...
			i = 1;
			para = 1;
			while (((lista[chave].unix != set.unix)||(lista[chave].quarto != set.quarto))&&(para == 1)){
			...
			para = 1;
			base = chave;
			while (((lista[chave].unix != tempo)||(set.quarto != lista[chave].quarto))&&(para == 1)){ //Esse While é DEUS!
	...
	fclose(arq);
	arq = fopen(frequencia,"w");
	for (i = 0; i < 24; i++){
		fprintf(arq, "%.0lf ", freq[i]);
	}
	fprintf(arq, "\n");
	fprintf(arq, "%d\n", total);
	fclose(arq);
...
void imprimir (enum Operacoes tipo, struct Info *lista){
	...
	switch(tipo){
		...
		fprintf (res, "O horário de pico está situado a partir de %d horas e vai até as %d\n horas ", lista[0].id, tempo);
