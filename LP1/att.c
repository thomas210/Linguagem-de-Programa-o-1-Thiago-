...
int organizar (struct Dados *lista, struct Dados *set, char escolha);
...
int organizar (struct Dados *lista, struct Dados *set, char escolha){
	...
	if (escolha == 'm'){
		...
		return 0;
			...
			chave = hash_a_div(set->unix, 0);
			while ((lista[chave].unix != (set->unix))||(set->quarto != lista[chave].quarto)){ //Esse While é DEUS!
				chave = hash_a_div(set->unix, i++);
				if (i >= 720){
					return 0;
				}
